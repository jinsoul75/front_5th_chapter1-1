...
`,d="/front_5th_chapter1-1",r=window.location.hash!=="",n=e=>{let t="";e==="/"?t=$():e==="/profile"?t=I({user:w()}):e==="/login"?t=L():t=P(),document.getElementById("root").innerHTML=t,e==="/login"?S():e==="/profile"&&A(),B()},i=(e,t)=>{if(e){const a=t.replace(d,"");window.location.hash=`${a}`}else window.history.pushState({},"",t)},E=e=>{n(e);const t=window.location.hash!=="",a=document.querySelector("#nav");a&&a.addEventListener("click",s=>{if(s.target.tagName==="A"){s.preventDefault();const o=s.target.getAttribute("href");console.log("a 클릭하면 href",o),i(t,`${d}${o}`),n(o)}})},S=()=>{if(localStorage.getItem("user")!==null)i(r,"/"),n("/");else{const e=document.getElementById("login-form");e&&e.addEventListener("submit",N)}},A=()=>{if(localStorage.getItem("user")===null)i(r,"/login"),n("/login");else{const e=document.getElementById("profile-form");e&&e.addEventListener("submit",O)}},B=()=>{const e=document.getElementById("logout");e&&e.addEventListener("click",j)},O=e=>{e.preventDefault();const t=document.getElementById("username").value,a=document.getElementById("email").value,s=document.getElementById("bio").value.trim(),o={username:t,email:a,bio:s};localStorage.setItem("user",JSON.stringify(o)),i(r,"/profile"),n("/profile")},N=e=>{e.preventDefault();const t={username:"testuser",email:"",bio:""};localStorage.setItem("user",JSON.stringify(t)),i(r,"/"),n("/")},j=()=>{localStorage.removeItem("user")},g=()=>{const t=window.location.pathname.replace(d,"")||"/";E(t)};window.addEventListener("load",g);window.addEventListener("popstate",g);
