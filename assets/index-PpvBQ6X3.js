...
`,h=t=>{const{profileImage:e,author:a,timeAgo:s,content:o,id:l}=t;return`
...
`,v=[{id:1,author:"홍길동",profileImage:"https://placehold.co/40",timeAgo:"5분 전",content:"오늘 날씨가 정말 좋네요. 다들 좋은 하루 보내세요!"},{id:2,author:"김철수",profileImage:"https://placehold.co/40",timeAgo:"15분 전",content:"새로운 프로젝트를 시작했어요. 열심히 코딩 중입니다!"},{id:3,author:"이영희",profileImage:"https://placehold.co/40",timeAgo:"30분 전",content:"오늘 점심 메뉴 추천 받습니다. 뭐가 좋을까요?"},{id:4,author:"박민수",profileImage:"https://placehold.co/40",timeAgo:"1시간 전",content:"주말에 등산 가실 분 계신가요? 함께 가요!"},{id:5,author:"정수연",profileImage:"https://placehold.co/40",timeAgo:"2시간 전",content:"새로 나온 영화 재미있대요. 같이 보러 갈 사람?"}],g=()=>localStorage.getItem("user")!==null,w=()=>JSON.parse(localStorage.getItem("user")||"null"),y=()=>{const t=`
  ...
  `;return m({content:t,isLoggedIn:g()})},$=({user:t})=>{const e=`
  ...
  `;return m({content:e,isLoggedIn:g()})},I=()=>`
...
`,u="/front_5th_chapter1-1",i=window.location.hash!=="",n=()=>{const t=window.location.hash!=="",e=t?window.location.hash.slice(1):window.location.pathname;console.log(" window.location.hash",window.location.hash),console.log("isHash",t),console.log("render",e);let a="";e==="/"?a=y():e==="/profile"?a=$({user:w()}):e==="/login"?a=I():a=L(),document.getElementById("root").innerHTML=a,e==="/login"?E():e==="/profile"?S():P()},r=(t,e)=>{console.log("navigateTo",u,e),t?window.location.hash=e:window.history.pushState({},"",`${u}${e}`)},E=()=>{if(localStorage.getItem("user")!==null)r(i,"/"),n();else{const t=document.getElementById("login-form");t&&t.addEventListener("submit",B)}},S=()=>{if(localStorage.getItem("user")===null)r(i,"/login"),n();else{const t=document.getElementById("profile-form");t&&t.addEventListener("submit",A)}},P=()=>{const t=document.getElementById("logout");t&&t.addEventListener("click",O)},A=t=>{t.preventDefault();const e=document.getElementById("username").value,a=document.getElementById("email").value,s=document.getElementById("bio").value.trim(),o={username:e,email:a,bio:s};localStorage.setItem("user",JSON.stringify(o)),r(i,"/profile"),n()},B=t=>{t.preventDefault();const e={username:"testuser",email:"",bio:""};localStorage.setItem("user",JSON.stringify(e)),r(i,"/"),n()},O=t=>{console.log("로그아웃 클릭"),t.preventDefault(),localStorage.removeItem("user"),r(i,"/login"),n()};document.addEventListener("click",t=>{if(t.target.tagName==="A"&&t.target.id!=="logout"){console.log("A 태그 클릭"),t.preventDefault();const e=t.target.getAttribute("href");window.history.pushState({},"",e),n()}});window.addEventListener("load",n);window.addEventListener("popstate",n);
