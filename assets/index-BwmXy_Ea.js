...
`,$=t=>{const{profileImage:e,author:l,timeAgo:s,content:o,id:a}=t;return`
...
`,r="/front_5th_chapter1-1";console.log("여긴 main.js");const n=()=>{const e=window.location.pathname.replace(r,"")||"/";console.log("path in main.js",e);let l="";e==="/"?l=P():e==="/profile"?l=E({user:S()}):e==="/login"?l=A():l=B(),document.getElementById("root").innerHTML=l,m(e)},N=()=>{const t=document.querySelector("#nav");t&&t.addEventListener("click",e=>{if(e.target.tagName==="A"){e.preventDefault();const l=e.target.getAttribute("href");console.log("이동을 여기로 ",`${r}${l}`),window.history.pushState({},"",`${r}${l}`),n()}})};window.addEventListener("load",()=>{N(),n()});window.addEventListener("popstate",n);
