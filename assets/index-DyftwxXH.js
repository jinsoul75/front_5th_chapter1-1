...
`,v=e=>{const{profileImage:t,author:a,timeAgo:s,content:o,id:l}=e;return`
...
`,y=[{id:1,author:"홍길동",profileImage:"https://placehold.co/40",timeAgo:"5분 전",content:"오늘 날씨가 정말 좋네요. 다들 좋은 하루 보내세요!"},{id:2,author:"김철수",profileImage:"https://placehold.co/40",timeAgo:"15분 전",content:"새로운 프로젝트를 시작했어요. 열심히 코딩 중입니다!"},{id:3,author:"이영희",profileImage:"https://placehold.co/40",timeAgo:"30분 전",content:"오늘 점심 메뉴 추천 받습니다. 뭐가 좋을까요?"},{id:4,author:"박민수",profileImage:"https://placehold.co/40",timeAgo:"1시간 전",content:"주말에 등산 가실 분 계신가요? 함께 가요!"},{id:5,author:"정수연",profileImage:"https://placehold.co/40",timeAgo:"2시간 전",content:"새로 나온 영화 재미있대요. 같이 보러 갈 사람?"}],g=()=>localStorage.getItem("user")!==null,w=()=>JSON.parse(localStorage.getItem("user")||"null"),$=()=>{const e=`
  ...
  `;return m({content:e,isLoggedIn:g()})},I=({user:e})=>{const t=`
  ...
  `;return m({content:t,isLoggedIn:g()})},L=()=>`
...
`,d="/front_5th_chapter1-1",r=window.location.hash!=="",n=e=>{let t="";e==="/"?t=$():e==="/profile"?t=I({user:w()}):e==="/login"?t=L():t=P(),document.getElementById("root").innerHTML=t,e==="/login"?S():e==="/profile"?A():B()},i=(e,t)=>{console.log("navigateTo",d,t),e?window.location.hash=t:window.history.pushState({},"",`${d}${t}`)},E=e=>{n(e);const t=window.location.hash!=="",a=document.querySelector("#nav");a&&a.addEventListener("click",s=>{if(console.log("로그아웃이 클릭이 안일어난다고 nav에서"),s.target.tagName==="A"){s.preventDefault();const o=s.target.id==="logout";console.log("isLogout",o);const l=s.target.getAttribute("href");o?(i(t,"/"),n("/")):(i(t,l),n(l))}})},S=()=>{if(localStorage.getItem("user")!==null)i(r,"/"),n("/");else{const e=document.getElementById("login-form");e&&e.addEventListener("submit",N)}},A=()=>{if(localStorage.getItem("user")===null)i(r,"/login"),n("/login");else{const e=document.getElementById("profile-form");e&&e.addEventListener("submit",O)}},B=()=>{const e=document.getElementById("logout");e&&e.addEventListener("click",j)},O=e=>{e.preventDefault();const t=document.getElementById("username").value,a=document.getElementById("email").value,s=document.getElementById("bio").value.trim(),o={username:t,email:a,bio:s};localStorage.setItem("user",JSON.stringify(o)),i(r,"/profile"),n("/profile")},N=e=>{e.preventDefault();const t={username:"testuser",email:"",bio:""};localStorage.setItem("user",JSON.stringify(t)),i(r,"/"),n("/")},j=()=>{console.log("로그아웃 클릭"),localStorage.removeItem("user")},f=()=>{const t=window.location.pathname.replace(d,"")||"/";E(t)};window.addEventListener("load",f);window.addEventListener("popstate",f);
