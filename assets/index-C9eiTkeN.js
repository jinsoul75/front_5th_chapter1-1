...
`,v=[{id:1,author:"홍길동",profileImage:"https://placehold.co/40",timeAgo:"5분 전",content:"오늘 날씨가 정말 좋네요. 다들 좋은 하루 보내세요!"},{id:2,author:"김철수",profileImage:"https://placehold.co/40",timeAgo:"15분 전",content:"새로운 프로젝트를 시작했어요. 열심히 코딩 중입니다!"},{id:3,author:"이영희",profileImage:"https://placehold.co/40",timeAgo:"30분 전",content:"오늘 점심 메뉴 추천 받습니다. 뭐가 좋을까요?"},{id:4,author:"박민수",profileImage:"https://placehold.co/40",timeAgo:"1시간 전",content:"주말에 등산 가실 분 계신가요? 함께 가요!"},{id:5,author:"정수연",profileImage:"https://placehold.co/40",timeAgo:"2시간 전",content:"새로 나온 영화 재미있대요. 같이 보러 갈 사람?"}],f=()=>localStorage.getItem("user")!==null,y=()=>JSON.parse(localStorage.getItem("user")||"null"),w=()=>{const t=`
  ...
  `;return m({content:t,isLoggedIn:f()})},$=({user:t})=>{const e=`
  ...
  `;return m({content:e,isLoggedIn:f()})},I=()=>`
...
`,d="/front_5th_chapter1-1",i=window.location.hash!=="",n=()=>{const e=window.location.hash!==""?window.location.hash.slice(1).replace(d,""):window.location.pathname.replace(d,"");let a="";e==="/"?a=w():e==="/profile"?a=$({user:y()}):e==="/login"?a=I():a=L(),document.getElementById("root").innerHTML=a,e==="/login"?E():e==="/profile"&&S(),P()},r=(t,e)=>{t?window.location.hash=e:window.history.pushState({},"",`${d}${e}`)},E=()=>{if(localStorage.getItem("user")!==null)r(i,"/"),n();else{const t=document.getElementById("login-form");t&&t.addEventListener("submit",B)}},S=()=>{if(localStorage.getItem("user")===null)r(i,"/login"),n();else{const t=document.getElementById("profile-form");t&&t.addEventListener("submit",A)}},P=()=>{const t=document.getElementById("logout");t&&t.addEventListener("click",O)},A=t=>{t.preventDefault();const e=document.getElementById("username").value,a=document.getElementById("email").value,s=document.getElementById("bio").value.trim(),o={username:e,email:a,bio:s};localStorage.setItem("user",JSON.stringify(o)),r(i,"/profile"),n()},B=t=>{t.preventDefault();const e={username:"testuser",email:"",bio:""};localStorage.setItem("user",JSON.stringify(e)),r(i,"/"),n()},O=t=>{t.preventDefault(),localStorage.removeItem("user"),r(i,"/login"),n()};document.addEventListener("click",t=>{if(t.target.tagName==="A"&&t.target.id!=="logout"){t.preventDefault();const e=t.target.getAttribute("href");window.history.pushState({},"",`${d}${e}`),n()}});window.addEventListener("load",n);window.addEventListener("popstate",n);
