...
`,c="/front_5th_chapter1-1",i=window.location.hash!=="",n=()=>{const e=window.location.hash!=="",t=e?window.location.hash.slice(1).replace(c,""):window.location.pathname.replace(c,"");console.log(" window.location.hash",window.location.hash),console.log("isHash",e),console.log("render",t);let a="";t==="/"?a=y():t==="/profile"?a=$({user:w()}):t==="/login"?a=I():a=L(),document.getElementById("root").innerHTML=a,t==="/login"?E():t==="/profile"?S():P()},r=(e,t)=>{console.log("navigateTo",c,t),e?window.location.hash=t:window.history.pushState({},"",`${c}${t}`)},E=()=>{if(localStorage.getItem("user")!==null)r(i,"/"),n();else{const e=document.getElementById("login-form");e&&e.addEventListener("submit",B)}},S=()=>{if(localStorage.getItem("user")===null)r(i,"/login"),n();else{const e=document.getElementById("profile-form");e&&e.addEventListener("submit",A)}},P=()=>{const e=document.getElementById("logout");e&&e.addEventListener("click",O)},A=e=>{e.preventDefault();const t=document.getElementById("username").value,a=document.getElementById("email").value,s=document.getElementById("bio").value.trim(),o={username:t,email:a,bio:s};localStorage.setItem("user",JSON.stringify(o)),r(i,"/profile"),n()},B=e=>{e.preventDefault();const t={username:"testuser",email:"",bio:""};localStorage.setItem("user",JSON.stringify(t)),r(i,"/"),n()},O=e=>{console.log("로그아웃 클릭"),e.preventDefault(),localStorage.removeItem("user"),r(i,"/login"),n()};document.addEventListener("click",e=>{if(e.target.tagName==="A"&&e.target.id!=="logout"){console.log("A 태그 클릭"),e.preventDefault();const t=e.target.getAttribute("href");window.history.pushState({},"",t),n()}});window.addEventListener("load",n);window.addEventListener("popstate",n);
